											...
											<tbody>
												...
												<tr>
													...
													<td>{{@$value->total_order}}</td>
														...
														@php
														$total_req_pending=DB::table('added_user')->where('admin_status','I')->where('user_to_temp_id',$value->id)->count();
														$req_time=DB::table('added_user')->where('admin_status','I')->where('user_to_temp_id',$value->id)->orWhere('admin_status','C')->where('user_to_temp_id',$value->id)->first();
														$total_completed=DB::table('added_user')->where('admin_status','C')->where('user_to_temp_id',$value->id)->count();
														@endphp
													...
													<td class="rm07">
														...
														<ul>
															...
															<li><a href="{{route('admin.all.print.req.card.list',$value->id)}}">Print Pending List <span class="badge"style="color: white; background-color: orange">{{$total_req_pending}}</span></a></li>

															<li><a href="{{route('admin.completed.print',$value->id)}}">Print Completed List <span class="badge" style="color: white; background-color: green">{{$total_completed}}</span></a></li>
